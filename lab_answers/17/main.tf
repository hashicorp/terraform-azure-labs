...
module "linuxservers" {
  ...
  public_ip_dns       = [random_pet.tflab_pet.id] // change to a unique name per datacenter region
  vnet_subnet_id      = azurerm_subnet.tflab_sn.id

  depends_on = [azurerm_resource_group.tflab_rg]
}